...
/// Vsock server for handling requests from parent instance
...
impl VsockServer {
    ...
    pub async fn start(&self) -> Result<()> {
        tracing::info!("Starting vsock server on port {}", VSOCK_PORT);

        // Create vsock listener
        let addr = VsockAddr::new(tokio_vsock::VMADDR_CID_ANY, VSOCK_PORT);
        let listener = VsockListener::bind(addr)
            .map_err(|e| EnclaveError::Network(format!("Failed to bind vsock: {}", e)))?;

        tracing::info!("Vsock server listening on port {}", VSOCK_PORT);
            ...
            match listener.accept().await {
                Ok((mut stream, peer_addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_connection(&mut stream, wallet_manager).await {
